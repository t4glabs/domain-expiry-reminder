...
GOOGLE_CHAT_WEBHOOK: str = os.getenv('GOOGLE_CHAT_WEBHOOK', '')
...
def make_report_for_google_chat() -> None:
    ...
    # Get webhook URL from CLI or environment variable
    webhook_url: str = CLI.google_chat_webhook if CLI.google_chat_webhook else GOOGLE_CHAT_WEBHOOK

    today: str = f'{datetime.now():%d.%m.%Y %H:%M}'
    hl: str = f'{"-" * 42}'
    message: str = ''
    message += f'*Domains Report  [ {today} ]*\n'
        ...
        for group, list_of_dict_data in WHOIS_TEXT_CHANGED_DOMAIN.items():
            ...
            for item_dict_data in list_of_dict_data:
                for domain, value in item_dict_data.items():
                    ...
                    dn: str = f'{domain:<20}'
                    txt: str = value.get('txt')
                    # Trim long text for Google Chat to avoid message size limits
                    if len(txt) > 350:
                        dt: str = value.get('dt')
                        str_domain_item: str = (
                            f'{i:>5}. {dn}{dt}\n\n'
    ...
    # Google Chat has a message size limit, truncate if needed
    if len(message) > 4000:
        message = message[:3900]
        message += '\n...\n_Message truncated due to size limit_'

    send_google_chat(message, webhook_url)
...
def process_cli():
    ...
    parent_group.add_argument(
        ...
        '--use-telegram',
        action='store_true',
        default=os.getenv('ENABLE_TELEGRAM', 'false').lower() in ('true', '1', 'yes'),
        help='Send a warning message through the Telegram (default is False, env: ENABLE_TELEGRAM)'
    )
    parent_group.add_argument(
        '-g',
        '--use-google-chat',
        action='store_true',
        default=os.getenv('ENABLE_GOOGLE_CHAT', 'false').lower() in ('true', '1', 'yes'),
        help='Send a warning message through Google Chat webhook (default is False, env: ENABLE_GOOGLE_CHAT)'
    )
    parent_group.add_argument(
        '-w',
        '--google-chat-webhook',
    ...
    parent_group.add_argument(
        '-e',
        '--email-to',
        default=os.getenv('EMAIL_TO', None),
        help='Send a warning message to email address (default is None, env: EMAIL_TO)',
        metavar='EMAIL'
    )
    parent_group.add_argument(
        '-subject',
    ...
    parent_group.add_argument(
        ...
        '--email-ssl',
        action='store_true',
        default=os.getenv('ENABLE_EMAIL_SSL', 'false').lower() in ('true', '1', 'yes'),
        help='Send email via SSL (default is False, env: ENABLE_EMAIL_SSL)'
    )
    parent_group.add_argument(
        '-auth',
        '--email-auth',
        action='store_true',
        default=os.getenv('ENABLE_EMAIL_AUTH', 'false').lower() in ('true', '1', 'yes'),
        help='Send email via authenticated SMTP (default is False, env: ENABLE_EMAIL_AUTH)'
    )
    parent_group.add_argument(
        '-starttls',
        '--email-starttls',
        action='store_true',
        default=os.getenv('ENABLE_EMAIL_STARTTLS', 'false').lower() in ('true', '1', 'yes'),
        help='Send email via STARTTLS (default is False, env: ENABLE_EMAIL_STARTTLS)'
    )
    parent_group.add_argument(
        '-oe',
